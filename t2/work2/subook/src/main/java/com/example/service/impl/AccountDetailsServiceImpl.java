    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        try {
            ...
            if (username.matches("\\d+")) {
                // 尝试按ID查询
                accountDetails = detailsMapper.selectById(Integer.parseInt(username));
            } 
            
            // 如果不是ID或ID查询未找到，则尝试按用户名或邮箱查询
            if (accountDetails == null) {
                accountDetails = detailsMapper.selectOne(
            ...
            throw new UsernameNotFoundException("查询错误: " + e.getMessage());
        }
    }
